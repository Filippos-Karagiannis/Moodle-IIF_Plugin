...
/**
 * Private i3f module utility functions
 *
 * @package    mod_i3f
 ...
 */
function i3f_appears_valid_url($url) {
 ...
 * @return string
 */
function i3f_fix_submitted_url($url) {
 ...
 */
function i3f_get_full_url($url, $cm, $course, $config=null) {
    ...
    if (preg_match('/^(\/|https?:|ftp:)/i', $fullurl) or preg_match('|^/|', $fullurl)) {
        ...
        $allowed = "[" . $letters . $latin . $digits . $symbols . $arabic . $math . $othernumbers . $geometric .
            $emojis . "]" . preg_quote(';/?:@=&$_.+!*(),-#%', '/');
        $fullurl = preg_replace_callback("/[^$allowed]/u", 'i3f_filter_callback', $fullurl);
    } else {
 ...
 * @return string
 */
function i3f_filter_callback($matches) {
 ...
 * @return void
 */
function i3f_print_header($url, $cm, $course) {
    global $PAGE, $OUTPUT;
 ...
 * @return void
 */
function i3f_print_heading($url, $cm, $course, $notused = false) {
    global $OUTPUT;
    echo $OUTPUT->heading(format_string($url->name), 2);
}

/**
 * Print i3f introduction.
 * @param object $url
 * @param object $cm
 * @param object $course
 ...
 * @return void
 */
function i3f_print_intro($url, $cm, $course, $ignoresettings=false) {
    global $OUTPUT;
    ...
    if ($ignoresettings or !empty($options['printintro'])) {
        if (trim(strip_tags($url->intro))) {
            ...
            echo format_module_intro('i3f', $url, $cm->id);
            echo $OUTPUT->box_end();
        }
    }
}
 ...
 */
function i3f_display_frame($url, $cm, $course) {
    ...
    if ($frame === 'top') {
        ...
        $exteurl = i3f_get_full_url($url, $cm, $course, $config);
        $navurl = "$CFG->wwwroot/mod/i3f/view.php?id=$cm->id&amp;frameset=top";
        $coursecontext = context_course::instance($course->id);
        $courseshortname = format_string($course->shortname, true, array('context' => $coursecontext));
        $title = strip_tags($courseshortname.': '.format_string($url->name));
        $framesize = $config->framesize;
        $modulename = s(get_string('modulename','i3f'));
...
/**
 * Print i3f info and link.
 * @param object $url
 * @param object $cm
 * @param object $course
 * @return does not return
 */
function i3f_print_workaround($url, $cm, $course) {
    global $OUTPUT;

    i3f_print_header($url, $cm, $course);
    i3f_print_heading($url, $cm, $course, true);
    i3f_print_intro($url, $cm, $course, true);

    $fullurl = i3f_get_full_url($url, $cm, $course);
    ...
    //obsolete code now since what is being posted is the description.
    //↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
     echo '<div class="urlworkaround">';
/*     $iiifurl = $url->externalurl;// the url object consists of alot of data, we only need the URL part
    $json = file_get_contents($iiifurl);// need to retrieve the json
    $json = trim($json); // trimming is recommended as a safe move
    $obj = json_decode($json,true); // decoding the json structure in an array
    foreach ($obj['metadata'] as $item) // displaying the string in a pretty way
    { 

        if (is_array($item['value'])) //check if we have arrays in there
        {
            echo '<strong>', $item['label'], '</strong>';

            foreach ($item['value'] as $values) //this could prove very costly in the longrun must convert to recursive
            {
                echo '<p>', $values, '</p>';
            }
        ...
        {
        echo '<strong>', $item['label'], '</strong>','<p>', $item['value'], '</p>';
        }
    } */
    $iiifurl = $url->externalurl;// the url object consists of alot of data, we only need the URL part
    $json = file_get_contents($iiifurl);// need to retrieve the json
 ...
 * @return does not return
 */
function i3f_display_embed($url, $cm, $course) {
    ...
    $clicktoopen = get_string('clicktoopen', 'i3f', $link);
 ...
 */
function i3f_get_final_display_type($url) {
    global $CFG;
 ...
 */
function i3f_get_variable_options($config) {
    global $CFG;

    $options = array();
    $options[''] = array('' => get_string('chooseavariable', 'i3f'));
    ...
    $options[get_string('modulename', 'i3f')] = array(
        'urlinstance'     => 'id',
        'urlcmid'         => 'cmid',
    ...
    $options[get_string('miscellaneous')] = array(
        ...
        'serverurl'       => get_string('serverurl', 'i3f'),
 ...
 */
function i3f_get_variable_values($url, $cm, $course, $config) {
    global $USER, $CFG;
 ...
 * @return string
 */
function i3f_get_encrypted_parameter($url, $config) {
    global $CFG;
 ...
 */
function i3f_guess_icon($fullurl, $size = null) {
    global $CFG;
    require_once("$CFG->libdir/filelib.php");
