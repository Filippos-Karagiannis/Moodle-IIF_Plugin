 ...
 * @since      Moodle 3.0
 */
class mod_i3f_external extends external_api {
    ...
    public static function view_url($urlid) {
        ...
        $url = $DB->get_record('i3f', array('id' => $params['urlid']), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($url, 'i3f');
    ...
    public static function get_urls_by_courses($courseids = array()) {
        ...
        if (!empty($params['courseids'])) {
            ...
            $urls = get_all_instances_in_courses("i3f", $courses);
            foreach ($urls as $url) {
                ...
                list($url->intro, $url->introformat) =
                    external_format_text($url->intro, $url->introformat, $context->id, 'mod_i3f', 'intro', null, $options);
                $url->introfiles = external_util::get_area_files($context->id, 'mod_i3f', 'intro', false, false);
