 ...
 */
class moodle1_mod_i3f_handler extends moodle1_resource_successor_handler {
    ...
    public function process_legacy_resource(array $data, array $raw = null) {
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_i3f', 'intro');
        $url['intro'] = moodle1_converter::migrate_referenced_files($url['intro'], $this->fileman);

        // write url.xml
        $this->open_xml_writer("activities/i3f_{$moduleid}/i3f.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid, 'moduleid' => $moduleid,
            'modulename' => 'i3f', 'contextid' => $contextid));
        $this->write_xml('i3f', $url, array('/i3f/id'));
        $this->xmlwriter->end_tag('activity');
        $this->close_xml_writer();

        // write inforef.xml
        $this->open_xml_writer("activities/i3f_{$moduleid}/inforef.xml");
